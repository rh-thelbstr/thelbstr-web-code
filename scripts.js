...
// Height Detection Helper for Short Screens (Lenovo ThinkPad fix)
function applyShortHeightClass() {
  const isShortHeight = window.innerHeight < 700;
  document.documentElement.classList.toggle('short-h', isShortHeight);
  
  // Debug logging
  console.log(`Screen height: ${window.innerHeight}px - ${isShortHeight ? 'SHORT' : 'NORMAL'} height detected`);
}

// Apply on load and resize
addEventListener('resize', applyShortHeightClass, { passive: true });
addEventListener('load', applyShortHeightClass, { passive: true });

// Apply immediately
applyShortHeightClass();

// GSAP Integration for short screens
if (typeof gsap !== 'undefined' && gsap.matchMedia) {
  const mm = gsap.matchMedia();
  
  // Only enable horizontal scroll animations on tall enough screens
  mm.add("(min-height: 701px)", () => {
    // Your existing GSAP horizontal scroll code goes here
    console.log('✅ Tall screen detected - enabling horizontal scroll animations');
    
    // Example GSAP code (replace with your actual code):
    /*
    gsap.to(wrapper, {
      x: -scrollDistance,
      ease: "none",
  ...
  // Disable animations on short screens
  mm.add("(max-height: 700px)", () => {
    console.log('📱 Short screen detected - disabling horizontal scroll');
